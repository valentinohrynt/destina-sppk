...
try:
    df = pd.read_csv('tourism_data_en.csv')
    print("File 'tourism_data_en.csv' berhasil dimuat.")
    ...
    print("ERROR: File 'tourism_data_en.csv' not found. Make sure it's in the same folder as app.py.")
...
def index():
    if df.empty and request.method == 'POST': # Jika df kosong dan ada POST, berarti CSV gagal load
         return render_template('index.html', cities=CITIES, error="Dataset could not be loaded. Please check the server console for errors regarding 'tourism_data_en.csv'.")
